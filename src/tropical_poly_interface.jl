...
"""
    get_coeff_by_exp(f::AbstractSignomial, e)

Return the coefficient for exponent vector `e`.
...
"""
    exponents(f::AbstractSignomial)

Return an iterable of all exponent vectors of `f` in lexicographic order.
For `SignomialStatic` this returns a `Vector{SVector{N,T}}`; for `SignomialMatrix`
it returns a `Vector{Vector{T}}` (one copy per column).
"""
...
"""
    coefficients(f::AbstractSignomial)

Return an iterable of all coefficients of `f` in lexicographic exponent order.
"""
