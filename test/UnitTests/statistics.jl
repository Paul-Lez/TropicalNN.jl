...
@testset verbose = true "Statistics" begin
    ...
    f = Signomial(Rational{BigInt}.([0, 1, 1]),
        [Rational{BigInt}.([0]), Rational{BigInt}.([1]), Rational{BigInt}.([2])];
        sorted = false)

    bds = bounds(f)
    @test bds == Dict{Any, Any}(Any[0 // 1, Rational{BigInt}[0]] => Any[Any[false]],
        Any[1 // 1, Rational{BigInt}[1]] => Any[Any[true]],
        Any[1 // 1, Rational{BigInt}[2]] => Any[Any[false]])

    vols = volumes(f)
    @test vols == Dict{Any, Any}(Any[0 // 1, Rational{BigInt}[0]] => Any[Inf],
        Any[1 // 1, Rational{BigInt}[1]] => Any[1.0],
        Any[1 // 1, Rational{BigInt}[2]] => Any[Inf])
    ...
    f = Signomial(
        Rational{BigInt}.([
            7915717918548363//9007199254740992, -7126386568116357//36028797018963968,
            5429561850506053//18014398509481984, 1797871556439715//9007199254740992,
            -7597859609031347//4503599627370496, 6086636570648303//4503599627370496]),
        [
            Rational{BigInt}.([
                6247149566212205//36028797018963968, 6402402109461593//9007199254740992]),
            Rational{BigInt}.([
                1560644898352435//1125899906842624, 308028398761065//562949953421312]),
            Rational{BigInt}.([
                5123970192597481//18014398509481984, 8573812228218511//9007199254740992]),
            Rational{BigInt}.([
                8850358768454271//72057594037927936, 6433642560136419//9007199254740992]),
            Rational{BigInt}.([
                3082055910080279//36028797018963968, -2092896630110503//2251799813685248]),
            Rational{BigInt}.([
                8780149460643975//18014398509481984, 5087317266127709//36028797018963968])];
        sorted = false)

    bds = bounds(f)
    @test bds == Dict{Any, Any}(
        Any[
            5429561850506053 // 18014398509481984, Rational{BigInt}[
                5123970192597481 // 18014398509481984, 8573812228218511 // 9007199254740992]] =>
            Any[Any[false]],
        Any[
            -7126386568116357 // 36028797018963968, Rational{BigInt}[
                1560644898352435 // 1125899906842624, 308028398761065 // 562949953421312]] =>
            Any[Any[false]],
        Any[
            -7597859609031347 // 4503599627370496, Rational{BigInt}[
                3082055910080279 // 36028797018963968, -2092896630110503 // 2251799813685248]] =>
            Any[Any[false]],
        Any[
            7915717918548363 // 9007199254740992, Rational{BigInt}[
                6247149566212205 // 36028797018963968, 6402402109461593 // 9007199254740992]] =>
            Any[Any[true]],
        Any[
            6086636570648303 // 4503599627370496, Rational{BigInt}[
                8780149460643975 // 18014398509481984, 5087317266127709 // 36028797018963968]] =>
            Any[Any[true]],
        Any[
            1797871556439715 // 9007199254740992, Rational{BigInt}[
                8850358768454271 // 72057594037927936, 6433642560136419 // 9007199254740992]] =>
            Any[Any[false]])

    vols = volumes(f)
    @test sort(collect(values(vols)))≈sort(collect(values(Dict{Any, Any}(
        Any[
            5429561850506053 // 18014398509481984, Rational{BigInt}[
                5123970192597481 // 18014398509481984, 8573812228218511 // 9007199254740992]] =>
            Any[Inf],
        ...
        Any[
            7915717918548363 // 9007199254740992, Rational{BigInt}[
                6247149566212205 // 36028797018963968, 6402402109461593 // 9007199254740992]] =>
            Any[83.85041171269546],
        Any[
            6086636570648303 // 4503599627370496, Rational{BigInt}[
                8780149460643975 // 18014398509481984, 5087317266127709 // 36028797018963968]] =>
            Any[17.689920806367773],
        Any[
            1797871556439715 // 9007199254740992, Rational{BigInt}[
                8850358768454271 // 72057594037927936, 6433642560136419 // 9007199254740992]] =>
            Any[Inf]))))
    ...
    @test Set(e_gradients[QQFieldElem[
        114891024932105469462376023557000 // 100773436731723900304245122318991, 251266862377778891172137271729659 // 134364582308965200405660163091988]]) ==
          Set(QQFieldElem[
        43693487181065715 // 5895790917138212, 19846348181041479 // 7290715696082942, -4000790818982757 // 8685640475027672])

    e_lengths = edge_lengths(f)
    @test sort(e_lengths["full"]) ≈ sort(Any[
        15.414689920279622, 9.544189179644512, 8.094242939512798, 0.4488256365358039,
        9.182503972830268, 5.7534317602946095, 6.168847902426598])

    e_directions = edge_directions(f)
    expected_e_directions = [
        [0.06842533619213997, 0.9976562400782107],
        [0.13372334048836676, 0.9910187022496761],
        [0.3448266493379887, 0.9386663847748762],
        [0.4116597011946916, -0.9113376379873144],
        [0.7504165727696047, -0.6609651785931847],
        [0.8268591498541684, -0.5624090560281209],
        [0.8757489917848431, 0.4827667173571838],
    ...
    @test all(zip(actual_e_directions, expected_e_directions)) do (actual, expected)
        actual ≈ expected
    end

    v_collection = vertex_collection(f)
    @test v_collection == Dict{Any, Any}(
        QQFieldElem[
            114891024932105469462376023557000 // 100773436731723900304245122318991, 251266862377778891172137271729659 // 134364582308965200405660163091988] =>
            3,
        QQFieldElem[
            3754412091190030378527856566377417 // 1037262225301565248686277949717669, -4353036857862963077097275417717051 // 1037262225301565248686277949717669] =>
            3,
        QQFieldElem[
            -3128137486873165258348604979472 // 231902538239625862028042208759, -195234271380496802632013468819 // 231902538239625862028042208759] =>
            3,
        QQFieldElem[
            896451073629643768901810677911431 // 829990503025703895676636601071105, 236587682011834540634606097327537 // 165998100605140779135327320214221] =>
            3,
        QQFieldElem[
            -52487495520323989250464295511660 // 4081230997476490799572483811083, 135808702710554091227834878243691 // 16324923989905963198289935244332] =>
            3,
        QQFieldElem[
            -483935954748453973516071434347234 // 122252722226343400701668229762973, -495975158221546378313136293612894 // 366758166679030202105004689288919] =>
            3)
