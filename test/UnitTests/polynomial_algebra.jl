...
@testset verbose = true "Polynomial Algebra Operations" begin
    ...
    @testset verbose = true "Polynomial Addition" begin
        ...
        g_flt = Signomial([R(3), R(4)], [[1.0, 0.0], [2.0, 0.0]]; sorted = false)
        h_flt = f_flt + g_flt
        @test length(h_flt) == 3

        # Test 3: Addition with overlapping monomials
        f2 = Signomial([R(5), R(3)], [[1//1, 0//1], [0//1, 1//1]]; sorted = false)
        ...
        @test TropicalNN.exponents(f4 + g4) == TropicalNN.exponents(g4 + f4)
        @test TropicalNN.coefficients(f4 + g4) == TropicalNN.coefficients(g4 + f4)

        # Test 5: Associativity
        h_test = Signomial([R(6)], [[1//1, 2//1]]; sorted = false)
        left = (f4 + g4) + h_test
        right = f4 + (g4 + h_test)
        ...
        @test empty isa Signomial{Rational{Int64}}
        @test Oscar.nvars(empty) == 2
        @test length(empty) == 0
    ...
    @testset verbose = true "Polynomial Multiplication" begin
        ...
        f_flt = Signomial([R(1), R(2)], [[1.0, 0.0], [0.0, 1.0]]; sorted = false)
        g_flt = Signomial([R(3)], [[1.0, 0.0]]; sorted = false)
        h_flt = f_flt * g_flt
        @test length(h_flt) == 2
    ...
    @testset verbose = true "Quicksum (Multi-polynomial Addition)" begin
        ...
        polys = [
            ...
            Signomial([R(3), R(4)], [[1//1, 0//1], [2//1, 0//1]]; sorted = false),
        ...
        polys_flt = [
            Signomial([R(1), R(2)], [[1.0, 0.0], [0.0, 1.0]]; sorted = false),
            Signomial([R(3)], [[1.0, 0.0]]; sorted = false),
            Signomial([R(4), R(5)], [[0.0, 1.0], [2.0, 0.0]]; sorted = false)
        ]
        result_flt = TropicalNN.quicksum(polys_flt)
        @test length(result_flt) == 3
        ...
        @test length(result_many) == 10  # All unique monomials
    ...
    @testset verbose = true "Tropical Rational Functions" begin
        ...
        num2 = Signomial([R(3)], [[1//1, 1//1]]; sorted = false)
        den2 = Signomial([R(0)], [[0//1, 0//1]]; sorted = false)
    ...
    @testset verbose = true "Polynomial Evaluation" begin
        ...
        num = Signomial([R(0), R(0)], [[1//1, 0//1], [0//1, 1//1]]; sorted = false)
        den = Signomial([R(0)], [[0//1, 0//1]]; sorted = false)
